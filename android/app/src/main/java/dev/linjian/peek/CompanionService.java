...
public class CompanionService extends Service {
    ...
    public static void handleCommandBody(Context ctx, String body, String serverUrl, String token) {
        try {
            ...
            if ("get_calendar_state".equals(action) || "upsert_calendar_event".equals(action) || "add_calendar_event".equals(action) || "delete_calendar_event".equals(action) || "list_calendar_notes".equals(action) || "add_calendar_note".equals(action) || "upsert_calendar_note".equals(action) || "react_calendar_note".equals(action) || "mark_calendar_notes_seen".equals(action)) {
    ...
    private static JSONObject performAction(Context ctx, String action, String app, String pkg, float x, float y, float x1, float y1, float x2, float y2, long duration, int hour, int minute, String title, String message, boolean vibrate, String serverUrl, String token, boolean skipUi, String targetText, String inputText, String match, int index, boolean append) {
        ...
        try {
            ...
            } else if ("get_calendar_state".equals(action) || "upsert_calendar_event".equals(action) || "add_calendar_event".equals(action) || "delete_calendar_event".equals(action) || "list_calendar_notes".equals(action) || "add_calendar_note".equals(action) || "upsert_calendar_note".equals(action) || "react_calendar_note".equals(action) || "mark_calendar_notes_seen".equals(action)) { JSONObject rr = CalendarState.handleCommand(ctx, cmd); ok = rr.optBoolean("ok", false); result = rr.optString("result", rr.toString());
            ...
            } else if ("show_pet".equals(action)) { AppPrefs.get(ctx).edit().putBoolean(AppPrefs.KEY_PIXEL_PET_ENABLED, true).apply(); PixelPetOverlay.sync(ctx); ok = true; result = "pixel_pet_shown";
            } else if ("hide_pet".equals(action)) { AppPrefs.get(ctx).edit().putBoolean(AppPrefs.KEY_PIXEL_PET_ENABLED, false).apply(); PixelPetOverlay.hide(); ok = true; result = "pixel_pet_hidden";
    ...
    @Override public void onDestroy() { running = false; PixelPetOverlay.hide(); DebugState.append(this, "服务已销毁/停止"); if (pollThread != null) pollThread.quitSafely(); super.onDestroy(); }
}
